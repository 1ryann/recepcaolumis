...
type AvailabilityEditorProps = {
  ...
  pending: boolean
  readOnly?: boolean
  // "self" is the professional editing their own agenda; "admin" is management editing
  // someone else's, where "Minha disponibilidade" and "você" would name the wrong person.
  audience?: 'self' | 'admin'
...
function AvailabilityEditorInner({ value, draft: draftValue, pending, readOnly = false, audience = 'self', onChange, onSave }: AvailabilityEditorProps) {
  ...
  const modeDescription = mode === 'INHERIT_GLOBAL'
    ? (admin ? 'O profissional está utilizando o horário de funcionamento do estabelecimento.' : 'Você está utilizando o horário de funcionamento do estabelecimento.')
    : (admin ? 'A agenda personalizada será cruzada com o horário do estabelecimento.' : 'Sua agenda personalizada será cruzada com o horário do estabelecimento.')
  ...
  return <section className="availability-editor panel">
    <div className="availability-editor-heading"><div><span className="eyebrow">Agenda semanal</span><h2>Disponibilidade semanal</h2><p>{modeDescription}</p></div><CalendarClock size={23} /></div>
    ...
    {value.existingReservationsOutsideAvailabilityCount > 0 && <div className="availability-warning" role="status"><AlertTriangle size={17} /><span>{admin ? 'Há' : 'Você possui'} {value.existingReservationsOutsideAvailabilityCount} agendamento(s) já existente(s) fora da nova disponibilidade. Esses agendamentos foram mantidos.</span></div>}
...
type ExceptionsEditorProps = { exceptions: AvailabilityExceptionDto[], pending: boolean, audience?: 'self' | 'admin', onCreate: (input: ExceptionDraft) => void | Promise<void>, onUpdate: (id: string, input: ExceptionDraft & { concurrencyToken: string }) => void | Promise<void>, onDelete: (id: string, concurrencyToken: string) => void | Promise<void> }

const emptyException: ExceptionDraft = { date: '', allDay: true, startTime: null, endTime: null, reason: '' }

export function ExceptionsEditor({ exceptions, pending, audience = 'self', onCreate, onUpdate, onDelete }: ExceptionsEditorProps) {
