...
export function Login({ audience = 'admin' }: { audience?: 'admin' | 'customer' | 'professional' }) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      navigate(current.mustChangePassword ? '/change-password' : homeForRoles(current.roles), { replace: true })
    } catch (error) {
    ...
    <main className="login-page">
      ...
      <section className="login-form-panel">
        <div className="login-form-wrap">
          ...
          {session.user ? <div><p>Sessão ativa: {session.user.displayName}</p><button className="primary-button" onClick={() => navigate(homeForRoles(session.user!.roles))}>Continuar na minha área</button><button className="secondary-button" onClick={async () => { try { await session.logout(); setPassword('') } catch { setError('Não foi possível confirmar a saída. Tente novamente.') } }}>Sair / Trocar conta</button></div> : <form onSubmit={submit}>
