...
export function ProfessionalApplicationStatus() {
  ...
  const statusClass = `lumis-login-title${approved ? ' lumis-status-positive' : rejected ? ' lumis-status-negative' : ''}`
  return <LumisPageShell className="lumis-login"><main className="lumis-login-card lumis-auth-surface is-wide"><LumisLogo className="lumis-login-logo" alt="LUMIS" width={124} height={38} /><span className="lumis-login-eyebrow">ÁREA DO PROFISSIONAL</span><h1 className={statusClass}>{approved ? 'Cadastro aprovado' : rejected ? 'Cadastro não aprovado' : 'Cadastro em análise'}</h1>{application && <p className="lumis-login-text"><strong>{application.name}</strong><br />{application.profession}<br /><small>Solicitado em {new Date(application.createdAt).toLocaleDateString('pt-BR')}</small></p>}<p className="lumis-login-text">{approved ? 'Seu acesso profissional foi liberado. Entre novamente para atualizar sua sessão.' : rejected ? 'Sua solicitação foi analisada e não foi aprovada. Procure a gerência caso precise de ajuda.' : 'Sua solicitação foi enviada e está aguardando aprovação da gerência.'}</p>{error && <div className="form-error">{error}</div>}<button type="button" className={approved ? 'primary-button lumis-login-submit' : 'secondary-button lumis-login-submit'} onClick={logout}><LogOut size={16}/> {approved ? 'Entrar na área profissional' : 'Sair'}</button></main></LumisPageShell>
}
